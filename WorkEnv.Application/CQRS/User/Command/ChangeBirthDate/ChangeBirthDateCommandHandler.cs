...
public class ChangeBirthDateCommandHandler(IUnitOfWork uof) : IRequestHandler<ChangeBirthDateCommand, Result<UserDTO>>
{
    public async Task<Result<UserDTO>> Handle(ChangeBirthDateCommand request, CancellationToken cancellationToken)
    {
        ...
        user.ChangeBirthDate(request.newBirthDate);
