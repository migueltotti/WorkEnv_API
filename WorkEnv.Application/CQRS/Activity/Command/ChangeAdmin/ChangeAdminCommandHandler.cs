...
public class ChangeAdminCommandHandler : IRequestHandler<ChangeAdminCommand, Result.Result>
{
    private readonly IUnitOfWork _uof;

    public ChangeAdminCommandHandler(IUnitOfWork uof)
    {
        _uof = uof;
    }
    ...
    public async Task<Result.Result> Handle(ChangeAdminCommand request, CancellationToken cancellationToken)
    {
        ...
        var newAdmin = await _uof.UserRepository.GetByIdAsync(request.newAdminId, cancellationToken);
